    ...
    <div style={{ padding: 24, color: 'var(--ink)', maxWidth: 920, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', borderBottom: '2px solid var(--line)', paddingBottom: 12, marginBottom: 18 }}>
        <div>
          ...
          <div className="h-display" style={{ fontSize: 32 }}>{t('foundry.title')}</div>
        </div>
        <div className="stat">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
        ...
        <div className="panel" style={{ padding: 14 }}>
          ...
          {lockedItems.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 460, overflow: 'auto' }}>
              {lockedItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <span style={{ fontSize: 18, color: RARITY_COLOR[item.rarity] }}>{item.glyph ?? '◆'}</span>
                    <span className="mono" style={{ fontSize: 10, letterSpacing: '0.12em', flex: 1 }}>{item.name}</span>
                    <span className="mono" style={{ fontSize: 10, color: 'var(--accent-1)' }}>{cost}</span>
                  </button>
                );
              })}
            </div>
...
function FoundryRow({ label, color, current, max, nextCost, essence, onBuy, desc }: RowProps) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderTop: '1px solid var(--line)' }}>
      ...
      <div className="mono" style={{ fontSize: 10 }}>{current}/{max}</div>
      <button
        className="btn btn-primary"
        onClick={onBuy}
        disabled={!can}
        style={{ padding: '6px 10px', fontSize: 10, minWidth: 100 }}
      >
